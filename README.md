...
| :--- | :--- |
|  `/set?key=K&value=V` | Write a key-value pair. Returns `leader_hint` if this node is not the leader. |
|  `/get?key=K` | Read a value. Returns `linearizable: true` if the read was confirmed by quorum. |
|  `/status` | Returns node state, term, commit index, applied index, and snapshot index. |
